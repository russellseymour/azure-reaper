...
namespace Azure.Reaper.Lib.Resources
{
  public class VirtualMachine
  {
    ...
    public Status SetPowerState()
    {
      ...
      logger.Debug("{0} - {1}: Power state - {2}", resourceGroup.GetName(), GetName(), powerState);
    ...
    private void ScheduleFromTags(bool fromResourceGroup)
    {
      ...
      if (hasTag)
      {
        ...
        {
          logger.Error("{0}: VM Schedule tag has an invalid format - '{1}'", resourceGroup.GetName(), (string) tagSchedule);
        }
      }
    }
    ...
    private void PermittedToRunDay(DateTime timeNow)
    {
      ...
      if (tagDaysPermitted)
      {
        logger.Information("{0}: Resource group contains permitted days for running machines - {1}", resourceGroup.GetName(), (string) tagDaysPermitted);
      ...
      if (tagDaysPermitted)
      {
        logger.Information("{0} - {1}: Virtual machine contains permitted days for running machines - {2}", resourceGroup.GetName(), GetName(), (string) tagDaysPermitted);
        days = tagDaysPermitted;
      }
      ...
      {
        logger.Information("{0}: Virtual machine is not permitted to run on a {1}: {2}", resourceGroup.GetName(), timeNow.DayOfWeek, GetName());
